...
export function emptyToStr(val: any) {
	return isEmpty(val) ? "" : val;
}

export function dateToYMD(val: any) {
	return isEmpty(val) ? "" : val.substring(0, 10);
}

// 医院名称缓存
let hospitalNameCache: string | null = null;

/**
 * 获取医院名称（公共方法，不需要登录）
 */
export async function getHospitalName(): Promise<string> {
	if (hospitalNameCache) {
		return hospitalNameCache;
	}

	try {
		const baseUrl = import.meta.env.PROD ? "/api" : "/dev";
		const url = `${baseUrl}/app/base/comm/param`;
				...
				success: (res: any) => {
					if (res.statusCode === 200) {
						...
						if (code === 1000) {
							...
							reject(new Error(message || "获取医院名称失败"));
						}
					} else {
		...
		if (typeof result === "string") {
			hospitalNameCache = result;
		} else if (result && typeof result === "object") {
			if (result.data?.content) {
				hospitalNameCache = result.data.content;
			} else if (result.content) {
				hospitalNameCache = result.content;
			} else if (result.data) {
				hospitalNameCache = result.data;
			} else {
				hospitalNameCache = String(result);
			}
		} else {
			hospitalNameCache = "";
		}
